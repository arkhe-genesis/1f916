...
class MatminerEngine:
    ...
    def featurize_many(self, structures: List[Structure]) -> np.ndarray:
        ...
        # OTIMIZAÇÃO: Conversão para float32 para reduzir RAM e acelerar parquet
...
class FeaturizerAgent(Agent):
    """Agente 1: Transforma estruturas em números e salva em disco."""
    def execute(self, state: Dict) -> Dict:
        logger.info(f"Extraindo features para {len(state['structures'])} estruturas...")
        engine = MatminerEngine()
        features = engine.featurize_many(state['structures'])

        # Salva o pool de features em .parquet antes de processar
        try:
            df = pd.DataFrame(features)
            parquet_path = "features.parquet"
            df.to_parquet(parquet_path)
            logger.info(f"Features salvas em {parquet_path}")
        except Exception as e:
            logger.warning(f"Não foi possível salvar features em parquet: {e}")

        state['X_matminer'] = features
        return state
